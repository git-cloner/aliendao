    ...
    def _completions_create(self, client, params):
        ...
        if params.get("stream", False) and "messages" in params and "functions" not in params:
            ...
            for message in params['messages']:
                if (message['role'] == "user" and message['name'] == "User_proxy"):
                    key = message['content']
                    break
            # check current agent
            message = params['messages'][-1]
            if (message['role'] == "user" and message['name'] == "Coder"): 
                role = "Product_manager"
            elif (message['role'] == "user" and message['name'] == "Product_manager"): 
                role = "User_proxy"
            elif (message['role'] == "system"):
                role ="User_proxy"
            elif (message['role'] == "user" and message['name'] == "User_proxy"): 
                role = "Coder"
            else :
                role = "User_proxy"
            if key != "" and role != "User_proxy" :
                temp_role =  "\n@agent " + role + "\n"
                ...
                if chunk.choices:
                    for choice in chunk.choices:
                        ...
                        if content is not None:
                            if key != "" and role != "User_proxy" :
                                response =  stream_buffer[key]["response"] 
                                response = response + temp_role + content
                                temp_role = ""
                                stream_buffer[key] = {"response": response, "stop": False, "history": [
            ...
            model = chunk.model.replace(
                "ChatGLM-6B", "gpt-4")  # hack for ChatGLM-6B
