...
if curl -I --silent --fail --location "http://61.133.217.142:20800/download/ollama/ollama-linux-${ARCH}.tgz${VER_PARAM}" >/dev/null ; then
    status "Downloading Linux ${ARCH} bundle"
    curl --fail --show-error --location --progress-bar \
        "http://61.133.217.142:20800/download/ollama/ollama-linux-${ARCH}.tgz${VER_PARAM}" | \
    ...
    "http://61.133.217.142:20800/download/ollama/ollama-linux-${ARCH}${VER_PARAM}"
...
if check_gpu lspci amdgpu || check_gpu lshw amdgpu; then
    if [ $BUNDLE -ne 0 ]; then
        ...
        curl --fail --show-error --location --progress-bar \
            "http://61.133.217.142:20800/download/ollama/ollama-linux-${ARCH}-rocm.tgz${VER_PARAM}" | \
            $SUDO tar -xzf - -C "$OLLAMA_INSTALL_DIR"
    ...
    curl --fail --show-error --location --progress-bar "http://61.133.217.142:20800/download/ollama/ollama-linux-amd64-rocm.tgz${VER_PARAM}" \
